...
from langchain_core.language_models.llms import LLM  # 基础LLM接口
from langchain.callbacks.streaming_stdout import StreamingStdOutCallbackHandler  # 流式回调
from langchain.callbacks.manager import CallbackManager  # 回调管理器
# 从vllm导入所需类
...
# 为VLLM创建一个LangChain兼容的包装类
class VLLMLangChain(LLM):
    """
    VLLM与LangChain兼容的适配器类
    """
    
    def __init__(self, model_name: str, temperature: float = 0.7, max_tokens: int = 2048, 
                 stop_tokens: List[str] = None, callback_manager=None, **kwargs):
        """初始化VLLM模型
        
        :param model_name: 模型名称路径
        :param temperature: 采样温度
        :param max_tokens: 最大生成令牌数
        :param stop_tokens: 停止令牌列表
        """
        ...
        self.model = VLLMLLM(model=model_name, **kwargs)
    ...
    def _call(self, prompt: str, stop: Optional[List[str]] = None, **kwargs) -> str:
        ...
        """
        # 合并停止标记
        stops = list(set((stop or []) + self.stop_tokens))
    ...
    def stream(self, prompt: str, stop: Optional[List[str]] = None, **kwargs) -> Generator[str, None, None]:
        """流式生成文本
        ...
        try:
            ...
            # 开始流式生成
            result_generator = self.model.generate([prompt], sampling_params, use_tqdm=False)
            
            # 初始化变量来跟踪已生成的文本
            last_output_len = 0
            
            # 处理流式输出
            for request_outputs in result_generator:
...
class RAGSystem:
    ...
    def __init__(self, config: Config):
        ...
        self.llm: Optional[VLLMLangChain] = None  # VLLM语言模型实例
    ...
    def _init_llm(self):
        """初始化VLLM大语言模型"""
        try:
            logger.info("🚀 正在初始化VLLM模型...")
            
            # 创建VLLM实例
            self.llm = VLLMLangChain(
                model_name=self.config.vllm_model_path,  # 使用配置中的模型路径
                temperature=self.config.llm_temperature,  # 温度参数
                max_tokens=self.config.llm_max_tokens,  # 最大生成token数
                stop_tokens=["<|im_end|>", "【答案】"],  # 停止标记
                tensor_parallel_size=self.config.vllm_tensor_parallel_size,  # 并行大小
                gpu_memory_utilization=self.config.vllm_gpu_memory_utilization,  # GPU内存使用率
                swap_space=self.config.vllm_swap_space,  # 交换空间
                enforce_eager=False,  # 不使用eager模式
            )
            ...
            logger.error(f"❌ VLLM模型初始化失败: {str(e)}")
            raise RuntimeError(f"无法初始化VLLM模型: {str(e)}")
    ...
    def query_model_with_kb(self, question: str) -> Tuple[str, List[Dict], Dict]:
        """结合知识库的非流式回答
        ...
        Returns:
            Tuple[str, List[Dict], Dict]: 答案、参考文献和元数据
        """
        return self.answer_query(question)
    
    def query_model_without_kb(self, question: str) -> str:
        """直接使用模型回答，不使用知识库
        ...
        try:
            ...
            logger.exception(f"直接模型调用错误: {str(e)}")
...
if __name__ == "__main__":
    ...
    # 1. 测试流式生成
    print("\n【流式生成测试】")
    for chunk in rag_system.stream_query_rag_with_kb(test_query):
        # 处理JSON数据
        try:
            ...
            if data["type"] == "content":
                ...
                print("\n找到以下参考资料:")
                for i, ref in enumerate(data["data"], 1):
                    print(f"  {i}. {ref['file']} (分数: {ref['score']:.2f})")
                print("\n生成回答中...\n")
    ...
    answer, references, metadata = rag_system.query_model_with_kb(test_query)
    end_time = time.time()
    ...
    print("\n【纯模型生成测试】")
    start_time = time.time()
    model_answer = rag_system.query_model_without_kb(test_query)
    end_time = time.time()
